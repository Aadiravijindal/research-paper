...
**4. TruthfulQA results are simulations, not live API runs.** [SIM] results in Sections 6.7–6.9 use the EPIC behavioral model calibrated to published single-agent baselines. The questions are real (Lin et al., 2022); the debate dynamics are modeled. Full empirical confirmation requires API execution (`epic_multimodel.py`). We do not present simulations as empirical findings; all are clearly labelled [SIM].
